...
- `GET /repos/{owner}/{repo}/contents/{path}` — read files from other repos
- `POST /repos/{owner}/{repo}/issues` — create issues in other repos
- `POST /repos/{owner}/{repo}/dispatches` — trigger workflows in other repos (event bridge, feature 13)
...
This is the multi-repo equivalent of "run the tests" — but across organizational boundaries.
...
- Monorepo support (that's a different problem — multiple packages in one repo)
- Automated cross-repo code changes (too dangerous — only alerts and proposals)
    ...
    if (file === "package.json") {
      for (const dep of dependents) {
        impacts.push({
          ...
          severity: "warning",
          details: `${dep} depends on ${currentRepo}. The package.json was modified — check for version compatibility.`,
        });
      }
    }
...
export async function alertAffectedRepos(
  ...
  for (const [targetRepo, repoImpacts] of byRepo) {
    ...
    const icon = { breaking: "🔴", warning: "🟡", info: "🔵" }[maxSeverity];
    ...
    const body = [
      ...
      "",
      "### Impact Details",
      ...repoImpacts.map(i => `- **${i.severity}**: ${i.reason} — ${i.details}`),
      "",
      "### Recommended Action",
      maxSeverity === "breaking"
        ? "⚠️ This may be a breaking change. Review your code for compatibility."
...
if (reposConfig.enabled && codeChanges.length > 0) {
  ...
  if (impacts.length > 0 && reposConfig.alertOnImpact) {
    ...
    agentText += `\n\n---\n⚠️ **Cross-repo impact detected**: ${impacts.length} downstream repo(s) may be affected. Alerts have been opened.`;
  }
}
```
...
- Modify an exported type in Repo B → verify an alert issue is created in Repo A
...
- Test with no org access → verify graceful degradation (feature disabled, not errored)

## Design Decisions

**Why alerts (issues) instead of automated cross-repo code changes?** Cross-repo code changes are extremely dangerous — the agent in Repo A doesn't have the full context of Repo B. An incorrect change could break Repo B's tests, introduce regressions, or conflict with in-flight work. Alerts give the Repo B team visibility and let them decide how to respond. If Repo B also has GitClaw, its agent can process the alert.
