...
GitHub Event (PR opened, issue comment, schedule)
    │
    ▼
Single Super YML Workflow (.github/workflows/github-gstack-intelligence-agent.yml)
    │
    ...
    ├── Step 4: Install (bun install --frozen-lockfile in .github-gstack-intelligence/)
    ├── Step 5: Indicate (indicator.ts — add 🚀 reaction)
    └── Step 6: Run Agent (agent.ts — route event, invoke pi with adapted skill prompt)
            │
            ├── Load skill prompt (from .github-gstack-intelligence/skills/{skill}.md)
...
```json
// .github-gstack-intelligence/package.json
{
  "dependencies": {
...
Install with Bun on the runner, invoke via `bun .github-gstack-intelligence/lifecycle/agent.ts`. This is the approach GMI uses today. It requires a JavaScript runtime but leverages the existing npm distribution channel and allows testing against unreleased versions.
...
- name: Download pi binary
  run: |
    curl -fsSL "https://github.com/japer-technology/github-pi-mono/releases/latest/download/pi-linux-x64.tar.gz" \
      -o /tmp/pi.tar.gz
...
The agent is configured via `.github-gstack-intelligence/.pi/settings.json`:
...
The gstack ETHOS principles are injected into the agent's system prompt via `.github-gstack-intelligence/.pi/APPEND_SYSTEM.md`:
...
| `Agent` (sub-agent) | ⚠️ Not natively available — decompose multi-agent skills into sequential steps within the single workflow |
...
- name: Run QA
  run: bun .github-gstack-intelligence/lifecycle/agent.ts --skill qa --url "${{ github.event.comment.body }}"
  env:
...
npx playwright test --headed=false --reporter=json .github-gstack-intelligence/scripts/navigate.ts
...
These gaps are filled by the lifecycle scripts and workflow configuration, not by modifying pi-mono. The agent is a runtime; the orchestration is github-gstack-intelligence's responsibility.
