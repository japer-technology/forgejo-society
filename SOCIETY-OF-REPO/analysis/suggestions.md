...
- `SOCIETY-OF-REPO/README.md` explains agencies, critics, censors, memory, workspace, and services.
- `FORGEJO/transition-plan/08-ai-agent-architecture.md` explains Forgejo repository classes, service accounts, and API-driven agent behaviour.
...
- `SOCIETY-OF-REPO/00-foundations/03-maturity-model.md` describes the ladder.
- `SOCIETY-OF-REPO/06-memory/klines/README.md` explains K-lines once they exist.
- `SOCIETY-OF-REPO/10-evolution/README.md` explains reinforcement once outcomes are happening.
...
`FORGEJO/transition-plan/08-ai-agent-architecture.md` gives API examples, but the repo does not yet seem to include starter implementations or live workflow templates. Both `.forgejo/` and `.github/` currently only expose `workflows-disabled/`.
...
- `SOCIETY-OF-REPO/02-protocols/05-settlement.md` defines the settlement schema and state flow.
- `FORGEJO/transition-plan/08-ai-agent-architecture.md` shows a four-step agent workflow.
...
- `FORGEJO/install/09-forgejo.md` covers install and backup creation.
- `FORGEJO/transition-plan/12-security-quotas-and-governance.md` starts disaster recovery and governance controls.
- `FORGEJO/install/06-prometheus-node-exporter.md` covers metrics exposure and basic continuity controls.
...
- `FORGEJO/transition-plan/08-ai-agent-architecture.md` says cloud escalation is policy-controlled.
- `FORGEJO/transition-plan/12-security-quotas-and-governance.md` defines secret scoping and approvals.
- `SOCIETY-OF-REPO/05-censors/` includes cloud-egress, credential, and PII exfiltration censors.
...
`FORGEJO/install/06-prometheus-node-exporter.md` gives host monitoring, which is useful, but the system you describe also needs cognitive observability.
...
`SOCIETY-OF-REPO/10-evolution/README.md` is directionally right, but still leaves a lot to judgment.
...
- `SOCIETY-OF-REPO/README.md` names a global workspace.
- `SOCIETY-OF-REPO/07-workspace/` defines the workspace areas.
